  ...
  return (
    <article className="ats-template ats-template--minimal">
      ...
      <section className="ats-section ats-section--tight">
        ...
        <p>{summary || 'Add a concise summary aligned to your target role.'}</p>
      </section>
      ...
      <section className="ats-section ats-section--tight">
        ...
        {experience.length ? experience.map((item, idx) => (
          <div className="ats-item" key={`minimal-exp-${idx}`}>
            ...
            <ul>
              {cleanList(item.highlights).map((line, lineIdx) => (
                <li key={`minimal-exp-line-${idx}-${lineIdx}`}>{line}</li>
              ))}
            </ul>
          </div>
      ...
      {projects.length ? (
        <section className="ats-section ats-section--tight">
          ...
          {projects.map((item, idx) => (
            <div className="ats-item" key={`minimal-project-${idx}`}>
              <h3>{item.name || 'Project'}</h3>
              {displayDateRange(item.startDate || '', item.endDate || '') ? <p className="ats-item__meta">{displayDateRange(item.startDate || '', item.endDate || '')}</p> : null}
              <ul>
                {cleanList(item.highlights).map((line, lineIdx) => (
                  <li key={`minimal-project-line-${idx}-${lineIdx}`}>{line}</li>
                ))}
              </ul>
            </div>
          ))}
      ...
      <section className="ats-section ats-section--tight">
        ...
        {education.length ? education.map((item, idx) => (
          <div className="ats-item" key={`minimal-edu-${idx}`}>
      ...
      {certifications.length ? (
        <section className="ats-section ats-section--tight">
          ...
          {certifications.map((item, idx) => (
            <div className="ats-item" key={`minimal-cert-${idx}`}>
