...
export type ActionVerbRuleResult = {
  ...
  percentage: number;
  passes: boolean;
  failingBulletIndexes: number[];
  failingBullets: ActionVerbFailure[];
  message: string;
};

export function analyzeActionVerbRule(
  bullets: string[],
  requiredRatio = ACTION_VERB_REQUIRED_RATIO,
): ActionVerbRuleResult {
  ...
  if (!normalizedBullets.length) {
    return {
      ...
      percentage: 0,
      passes: true,
      failingBulletIndexes: [],
      failingBullets: [],
      message: '',
    };
  }

  const failures: ActionVerbFailure[] = [];
  let strongCount = 0;
  for (let index = 0; index < normalizedBullets.length; index += 1) {
    const bullet = normalizedBullets[index];
    const verdict = evaluateBulletStarter(bullet);
    if (verdict.accepted) {
      strongCount += 1;
      continue;
    }
    failures.push({
      ...
      suggestions: buildStarterSuggestions(bullet),
    });
  }

  const totalBullets = normalizedBullets.length;
  const requiredStrongBullets = Math.ceil(totalBullets * requiredRatio);
  const remainingToPass = Math.max(0, requiredStrongBullets - strongCount);
  const percentage = totalBullets ? Math.round((strongCount / totalBullets) * 100) : 0;
  ...
  return {
    ...
    percentage,
    passes,
    failingBulletIndexes: failures.map((item) => item.index),
    failingBullets: failures,
    message: passes
      ? `Strong action verb coverage is ${percentage}% (${strongCount}/${totalBullets}) bullets.`
      : `At least ${thresholdPercent}% of experience bullets must start with a strong action verb. Currently ${percentage}% (${strongCount}/${totalBullets}). Fix ${remainingToPass} more bullet${remainingToPass === 1 ? '' : 's'} to reach ${thresholdPercent}%.`,
  };
}
...
export function normalizeVerbToken(token: string) {
...
export function isWeakStarter(bullet: string) {
...
export function isAcceptedStrongStarter(bullet: string) {
  return evaluateBulletStarter(bullet).accepted;
}

export function buildStarterSuggestions(bullet: string, limit = 6) {
...
function evaluateBulletStarter(bullet: string): { accepted: boolean; reason: 'weak_starter' | 'not_strong_enough' } {
  ...
  if (isWeakStarter(normalized)) {
    return { accepted: false, reason: 'weak_starter' };
  }
  const firstToken = normalized.split(/\s+/)[0] || '';
  const lemma = normalizeVerbToken(firstToken);
  if (lemma && STRONG_VERB_SET.has(lemma)) {
    return { accepted: true, reason: 'not_strong_enough' };
  }
  if (looksLikeStrongVerbHeuristic(firstToken, normalized)) {
    return { accepted: true, reason: 'not_strong_enough' };
  }
  return { accepted: false, reason: 'not_strong_enough' };
}
