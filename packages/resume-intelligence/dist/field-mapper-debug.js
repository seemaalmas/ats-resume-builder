...
function mapSkills(sections) {
    ...
    const tokens = lines
        ...
        if (words.length > 4)
            return false;
        if (/^(and|or|in|on|with|for|to|of|the)$/i.test(words[0] || ''))
            return false;
        if (/[!?]/.test(token))
            return false;
        ...
        if (/@|https?:\/\/|www\./i.test(token))
            return false;
        if (CONTACT_LABEL_RE.test(token))
            return false;
        if (NAME_BLOCKLIST_RE.test(token))
            return false;
        if (isLikelyNameLine(token))
            return false;
        if (ROLE_HINT_RE.test(token) && token.split(/\s+/).length >= 3)
            return false;
        if (/^\#/.test(token))
            return false;
...
function mapExperience(parsed) {
    ...
    const assignDatesToCurrentOrRecent = (startDate, endDate) => {
    ...
    const appendBulletToNearestBlock = (bullet) => {
        if (current) {
    ...
    for (const rawLine of source) {
        ...
        if (!normalizedLine)
            continue;
        const heading = (0, section_normalizer_js_1.normalizeHeading)(normalizedSourceLine);
        ...
        if (companyHeading) {
            ...
            if (current && !current.company)
                current.company = companyHeading;
            continue;
        }
        const fullHeader = parseExperienceHeader(normalizedLine); if (fullHeader) console.log("TRACE: fullHeader="+JSON.stringify(fullHeader));
        ...
        if (!current)
            continue;
        console.log("TRACE: line="+JSON.stringify(normalizedLine)+" current="+(current?"exists(role="+current.role+", co="+current.company+", hl="+current.highlights.length+")":"null")); if (normalizedLine.length > 10)
            current.highlights.push(normalizedLine);
    }
...
function mapProjects(sections) {
    ...
    for (const line of lines) {
        ...
        if (!current)
            current = { name: 'Project', role: '', startDate: '', endDate: '', highlights: [] };
        if (line.startsWith('-'))
...
function mapHeader(lines) {
    ...
    for (let i = 0; i < Math.min(cleanLines.length, 16); i += 1)
        candidateIndexes.add(i);
    let bestName = '';
    let bestNameIndex = -1;
    ...
    const links = cleanLines
        ...
        if (matches.length)
            return matches;
        if (/linkedin\.com|github\.com|portfolio|website/i.test(line))
            return [line];
        return [];
    })
...
function guessTitle(lines, header) {
    ...
    const roleCandidate = lines
        ...
        if (!line)
            return false;
        if (isBlockedTitleValue(line))
            return false;
        if (line.length > 140 || /@|https?:\/\/|www\./i.test(line))
            return false;
        ...
        if (/\b\d{1,2}[/-]\d{4}\b/.test(line) || /\b(20\d{2}|19\d{2})[-/]\d{1,2}\b/.test(line))
            return false;
        return true;
    });
    if (roleCandidate) {
...
function isLikelyNameLine(line) {
    ...
    if (!cleaned || cleaned.length > 56 || cleaned.length < 3)
        return false;
    if (isBlockedTitleValue(cleaned))
        return false;
    if (NAME_BLOCKLIST_RE.test(cleaned))
        return false;
    if (CONTACT_LABEL_RE.test(cleaned))
        return false;
...
function extractHeadline(lines, nameIndex) {
    ...
    for (let idx = nameIndex + 1; idx < searchLimit; idx += 1) {
        ...
        if (!candidate)
            continue;
        if (isLikelyHeadlineLine(candidate))
            return candidate;
    }
    return '';
}
function isLikelyHeadlineLine(line) {
    ...
    if (/\d{6,}/.test(cleaned))
        return false;
    if (cleaned.length < 8 || cleaned.length > 140)
        return false;
...
function extractLocation(lines) {
    for (let i = 0; i < lines.length; i += 1) {
        ...
        if (/address\s*:/i.test(line)) {
            ...
            for (let j = i + 1; j < Math.min(lines.length, i + 3); j += 1) {
                ...
                if (CONTACT_LABEL_RE.test(next) || (0, section_normalizer_js_1.normalizeHeading)(lines[j]))
                    break;
                return next;
            }
        }
    ...
    const fallback = lines.find((line) => {
        ...
        if (/\b\d{5,6}\b/.test(cleaned) && /,/.test(cleaned))
            return true;
...
function isBlockedTitleValue(value) {
    ...
    if (!normalized)
        return true;
    if (TITLE_BLOCKLIST.has(normalized))
        return true;
    return Boolean((0, section_normalizer_js_1.normalizeHeading)(value));
}
function normalizeHeadlineForTitle(value) {
...
function getUnmappedText(sections) {
...
function sanitizeExperienceForStrictSave(items) {
    const cleanItems = [];
    const rejected = [];
...
function sanitizeEducationForStrictSave(items) {
    const cleanItems = [];
    const rejected = [];
    for (const item of items) {
        ...
        if (!hasCore)
            continue;
        const strictValid = ((institution.length >= 2 || degree.length >= 2));
        if (!strictValid) {
...
function mergeUnmappedText(base, additions) {
    const merged = [base, ...additions]
        .map((line) => cleanLooseText(line))
        .filter(Boolean);
    return uniqueLines(merged).join('\n').trim();
}
function buildRejectedLine(prefix, parts) {
...
function buildExperienceSource(parsed) {
    const sectionLines = [
    ...
    if (sectionLines.length) {
        ...
        if (hasRoleCompany) {
            ...
            if (spillover.length)
                return [...sectionLines, ...spillover];
        }
        return sectionLines;
    }
    const firstExperienceHeading = parsed.lines.findIndex((line) => (0, section_normalizer_js_1.normalizeHeading)(line) === 'experience');
...
function parseExperienceHeader(line) {
    if (looksLikeEducationRoleLine(line))
        return null;
...
function parseRoleWithOptionalDates(line, currentCompany) {
    if (looksLikeEducationRoleLine(line))
        return null;
    const dates = extractDates(line);
    ...
    if (!currentCompany || !role || !looksLikeRole(role) || !looksLikeRoleTitle(role))
        return null;
    return {
...
function extractBulletLine(line) {
...
function looksLikeLocationFragment(value) {
    ...
    for (const delimiter of [' — ', ' – ', ' - ', ' | ', ' â€” ']) {
        if (normalized.includes(delimiter)) {
            ...
            if (parts.length >= 2) {
                ...
                if (leftLooksCompany && rightLooksRole)
                    return { role: right, company: cleanCompanyName(left) };
                if (rightLooksCompany && leftLooksRole)
                    return { role: left, company: cleanCompanyName(right) };
                if (leftLooksRole && !rightLooksRole)
                    return { role: left, company: cleanCompanyName(right) };
                if (rightLooksRole && !leftLooksRole)
                    return { role: right, company: cleanCompanyName(left) };
                if (leftLooksCompany && !rightLooksCompany)
                    return { role: right, company: cleanCompanyName(left) };
                if (rightLooksCompany && !leftLooksCompany)
                    return { role: left, company: cleanCompanyName(right) };
                return { role: left, company: cleanCompanyName(right) };
            }
        }
    }
    if (looksLikeCompany(normalized))
        return { role: '', company: cleanCompanyName(normalized) };
    return { role: normalized, company: '' };
}
function parseRoleCompanyPair(line) {
    const split = splitRoleCompany(line);
    const role = cleanLooseText(split.role);
    const company = cleanCompanyName(split.company);
    if (!role || !company)
        return null;
    if (!looksLikeRole(role) && !looksLikeRoleTitle(role))
        return null;
...
function looksLikeEducationRoleLine(line) {
    const cleaned = cleanLooseText(line);
    if (!cleaned)
        return false;
...
function mergeExperienceByCompany(experience) {
    const map = new Map();
    for (const item of experience) {
        if (!isMeaningfulExperience(item))
            continue;
        const company = cleanCompanyName(item.company);
        const role = cleanLooseText(item.role);
        const startDate = normalizeDateToken(cleanLooseText(item.startDate));
...
function sortExperienceChronological(experience) {
...
function isDateLine(line) {
    ...
    if (/\s+-\s+|\bto\b|–|—|â€”|â€”/i.test(line))
        return true;
    // YYYY-MM - YYYY-MM format (dashes within dates and between dates)
    if (/\b(20\d{2}|19\d{2})[-/]\d{1,2}\b.*\s+-\s+.*\b(20\d{2}|19\d{2})/i.test(line))
        return true;
    // MM/YYYY - MM/YYYY format
    if (/\b\d{1,2}[/-](20\d{2}|19\d{2})\b.*\s+-\s+.*\b\d{1,2}[/-](20\d{2}|19\d{2})/i.test(line))
        return true;
    return false;
}
...
function extractDates(line) {
    ...
    if (!match) {
        ...
        if (bareRange)
            return { start: normalizeDateToken(bareRange[1]), end: normalizeDateToken(bareRange[2]) };
        return { start: '', end: '' };
    }
    return {
...
function normalizeDateToken(token) {
    ...
    if (/present|current|now|till\s*date/i.test(clean))
        return 'Present';
    return clean;
}
