...
exports.computeExperienceLevel = computeExperienceLevel;
function computeExperienceLevel(input) {
    const meaningful = input.experience.filter(isMeaningfulExperience);
    const signals = {
        roleCount: meaningful.length,
        distinctCompanyCount: new Set(meaningful.map((entry) => normalizeCompany(entry.company)).filter(Boolean)).size,
...
function isMeaningfulExperience(entry) {
    return Boolean(entry.company.trim() ||
        entry.role.trim() ||
...
function estimateTotalMonths(experience) {
    let total = 0;
