...
function looksLikeEducationInstitutionLine(line) {
    ...
    if (/\b(university|college|school|institute|academy|polytechnic|conservatory)\b/i.test(cleaned))
        return true;
    // Match common Indian institution abbreviations (IIT, IIIT, NIT, BITS, etc.)
    if (/\b(IIT|IIIT|NIT|BITS|SPPU|VTU|JNTU|AKTU|MIT|DTU|NSIT)\b/.test(cleaned))
        return true;
    // Reject dates, degrees, short abbreviations, and bullet points
    if (cleaned.length < 3)
        return false;
    if (isStandaloneDateLine(cleaned))
        return false;
    if (looksLikeEducationDegreeLine(cleaned))
        return false;
    if (/^[-•*]/.test(cleaned))
        return false;
    ...
    if (words.length >= 1 && words.length <= 8) {
        ...
        if (titleCaseWords.length >= Math.ceil(words.length * 0.5))
            return true;
    }
    return false;
}
