...
exports.enhanceExperienceExtraction = enhanceExperienceExtraction;
exports.extractExperienceFromWorkExperienceSection = extractExperienceFromWorkExperienceSection;
...
function enhanceExperienceExtraction(input) {
    ...
    const candidates = blocks
        ...
        .filter((item) => Boolean(item && (item.company || item.role)));
    return candidates.length ? candidates : trimmedCurrent;
}
function extractExperienceLines(rawText, parsed) {
    if (parsed) {
        const sectionLines = [
    ...
    if (startIndex >= 0) {
        ...
        if (splitIndex >= 0)
            return tail.slice(0, splitIndex);
        if (tail.length)
            return tail;
    }
...
function filterLikelyExperienceLines(lines) {
    return lines.filter((line) => {
        ...
        if (line.startsWith('-') && line.length > 2)
            return true;
        if (ROLE_HINT_RE.test(line) && line.split(' ').length <= 6)
            return true;
        if (COMPANY_HINT_RE.test(line))
            return true;
...
function shouldUseWorkExperienceFallback(current, rawText) {
    if (!current.length)
        return true;
    if (current.length === 1 && looksLikeDateFragment(current[0]))
        return true;
...
function extractExperienceFromWorkExperienceSection(rawText, parsed) {
    if (!rawText && !parsed?.lines?.length)
        return [];
    ...
    if (!normalizedLines.length)
        return [];
    const startIndex = normalizedLines.findIndex((line) => (0, section_normalizer_js_1.normalizeHeading)(line) === 'experience');
    if (startIndex < 0)
        return [];
    const blockLines = [];
...
function parseWorkExperienceBlock(lines) {
    const entries = [];
    let index = 0;
...
function parseCompanyRolePair(companyLine, roleLine) {
...
function parseInlineCompanyRole(line) {
...
function shouldBreakForNextJob(line, nextLine) {
    if (!line)
        return false;
    if (/^achievements:?$/i.test(line))
        return false;
    if (parseInlineCompanyRole(line))
        return true;
    if (nextLine && hasDateRange(nextLine) && looksLikeCompanyLine(line))
        return true;
    return false;
}
...
function matchDateRangeSegment(line) {
...
function collectDateRangeMatches(text) {
    if (!text)
        return [];
...
function looksLikeDateFragment(entry) {
